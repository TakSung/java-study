...
| **java-runner** | `agent/skills/java-runner.md` | mvn, java 실행, 자바 테스트, 컴파일 | Run Java files, execute tests with Maven, compile, and package |
...
**File**: `agent/skills/java-runner.md`

**Function**: Java execution, testing, and validation using Maven

**Key Features**:
- Run main class via `mvn exec:java`
- Execute tests with `mvn test`
- Compile and syntax check with `mvn compile`
- Clean build artifacts with `mvn clean`
...
자바 프로젝트 실행해줘
mvn으로 테스트 돌려줘
이 파일 컴파일 오류 없는지 확인해줘
```

**Tools**: Bash, Read | **Platform**: Linux/macOS/Windows
